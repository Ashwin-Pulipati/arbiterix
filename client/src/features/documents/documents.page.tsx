...
export function DocumentsPage() {
  ...
  const onRequestDeleteCallback = useCallback(
    async (id: number) => {
      try {
  ...
  const onUndoRequestDeleteCallback = useCallback(
    async (id: number) => {
      try {
        ...
        toast.error(
          e instanceof Error ? e.message : "Failed to undo deletion request",
        );
      }
    },
    [undoRequestDeleteDoc],
